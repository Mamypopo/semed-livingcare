  ...
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
      ...
      <div class="flex items-center gap-2">
        <input v-model.trim="query" @input="onFilterInput" type="text" placeholder="ค้นหา ชื่อ/อีเมล..." class="px-3 py-2 text-sm border border-gray-200 rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 w-56 hover:border-emerald-300 focus:outline-none" />
        ...
        <Listbox v-model="roleOption" as="div" class="relative">
          ...
          <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
            <ListboxOptions class="absolute right-0 mt-2 z-50 p-2 shadow-xl bg-white rounded-xl border border-gray-100 w-48 focus:outline-none">
              <ListboxOption v-for="opt in roleOptions" :key="String(opt.value)" :value="opt" v-slot="{ active, selected }">
        ...
        <button class="px-4 py-2 text-sm bg-emerald-600 text-white rounded-md hover:bg-emerald-700 focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2" @click="openCreate">เพิ่มผู้ใช้</button>
      </div>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            <template v-if="!loading">
              ...
              <td class="px-4 py-2 text-sm text-gray-700 whitespace-nowrap">{{ formatDate(user.createdAt) }}</td>
...
export default {
  ...
  components: { 
    ...
    Listbox, ListboxButton, ListboxOptions, ListboxOption, UserModal 
  },
  setup() {
  ...
  methods: {
    ...
    async handleSave(data) {
      ...
      const confirm = await Swal.fire({
        ...
        text: isEdit ? `ต้องการบันทึกการเปลี่ยนแปลงของ "${data.name}" หรือไม่` : `ต้องการสร้างผู้ใช้ "${data.name}" หรือไม่`,
        icon: 'question',
