    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div>
          ...
          <div class="text-lg font-semibold text-gray-900">(OPD) {{ queueData?.queueNumber || '-' }}</div>
          <div class="text-sm text-gray-600 mt-2">แผนก: {{ queueData?.department?.name || '-' }}</div>
        </div>

        <!-- Date & Priority -->
        <div>
          <div class="text-sm text-gray-600 mb-1">วันที่</div>
          <div class="text-lg font-semibold text-gray-900">{{ formatDateTime(queueData?.createdAt) }}</div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-9 gap-6">
      ...
      <div class="lg:col-span-3 bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div class="flex items-start gap-4">
          <div class="w-12 h-12 bg-gradient-to-r from-purple-400 to-teal-400 rounded-full flex items-center justify-center flex-shrink-0">
          ...
          <div class="flex-1">
            ...
            <div class="text-sm font-medium text-gray-700 mt-2">
              {{ patientData?.prefix || '' }} {{ patientData?.first_name || '' }} {{ patientData?.last_name || '' }}
            </div>
            ...
            <div class="text-sm text-gray-600">
              เพศ: {{ patientData?.gender === 'M' ? 'ชาย' : patientData?.gender === 'F' ? 'หญิง' : '-' }}
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div class="space-y-3">
          ...
          <div>
            ...
            <div class="text-sm font-medium text-gray-900">{{ patientData?.insuranceType?.name || 'อื่นๆ (รอยืนยันสิทธิ)' }}</div>
          </div>
        </div>
      </div>
      ...
      <div class="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          <div class="col-span-2">
            ...
            <div class="text-sm font-medium text-gray-900">{{ patientData?.blood_group || 'ไม่ระบุ' }}</div>
          </div>
        </div>
      </div>
      ...
      <div class="lg:col-span-2 bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div class="space-y-3">
          ...
          <div>
            ...
            <div class="text-sm font-medium text-gray-900">{{ patientData?.mental_health_history || '-' }}</div>
          </div>
          <div>
            <div class="text-sm text-gray-600 mb-1">โรคประจำตัว</div>
            <div class="text-sm font-medium text-gray-900">{{ patientData?.chronic_diseases || '-' }}</div>
          </div>
        </div>
      </div>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">แพทย์ผู้ตรวจ</th>
              <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">การวินิจฉัยโรค</th>
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr v-if="loadingHistory">
              <td colspan="5" class="px-6 py-4 text-center">
                <div class="flex items-center justify-center">
                  <div class="animate-spin rounded-full h-6 w-6 border-2 border-gray-300 border-t-emerald-500"></div>
            ...
            <tr v-else v-for="record in medicalHistory" :key="record.id" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ formatDateTime(record.createdAt) }}</td>
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{{ record.doctor?.name || '-' }}</td>
