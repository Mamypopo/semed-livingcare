  ...
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
      ...
      <div class="flex items-center gap-2">
        ...
        <button class="px-4 py-2 text-sm bg-emerald-600 text-white rounded-md hover:bg-emerald-700 focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2" @click="openCreate">
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              <th @click="toggleSort('hn')" class="px-4 py-2 text-left text-xs font-semibold cursor-pointer select-none text-gray-600">
                <span class="inline-flex items-center">HN
                  <ChevronUp v-if="sort==='hn' && order==='asc'" class="w-3.5 h-3.5 ml-1 text-emerald-600" />
                  <ChevronDown v-else class="w-3.5 h-3.5 ml-1 text-emerald-600" />
                </span>
              </th>
              <th @click="toggleSort('first_name')" class="px-4 py-2 text-left text-xs font-semibold cursor-pointer select-none text-gray-600">
                <span class="inline-flex items-center">ชื่อ-นามสกุล
                  <ChevronUp v-if="sort==='first_name' && order==='asc'" class="w-3.5 h-3.5 ml-1 text-emerald-600" />
                  <ChevronDown v-else class="w-3.5 h-3.5 ml-1 text-emerald-600" />
                </span>
              </th>
          ...
          <tbody class="divide-y divide-gray-100">
              ...
              <tr v-for="patient in patients" :key="patient.id" class="hover:bg-gray-50">
                ...
                <td class="px-4 py-2 text-sm text-gray-900">
                  <div class="flex items-center">
                    <div>
                      <div class="font-medium">{{ patient.prefix }}{{ patient.first_name }} {{ patient.last_name }}</div>
                      <div v-if="patient.nickname" class="text-xs text-gray-500">({{ patient.nickname }})</div>
                    </div>
                  </div>
                </td>
                ...
                <td class="px-4 py-2 text-sm">
                  <span v-if="patient.patientGroup" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium" :style="{ backgroundColor: patient.patientGroup.color + '20', color: patient.patientGroup.color }">
                    {{ patient.patientGroup.name }}
                  </span>
                  <span v-else class="text-gray-400">-</span>
                </td>
                <td class="px-4 py-2 text-sm">
                  <span :class="patient.isActive ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-600'" class="px-2 py-1 rounded-full text-xs font-medium">
                    {{ patient.isActive ? 'ใช้งาน' : 'ปิดใช้งาน' }}
                  </span>
                </td>
                <td class="px-4 py-2 text-sm text-gray-700 whitespace-nowrap">{{ formatDate(patient.createdAt) }}</td>
            ...
            <tr v-if="!loading && patients.length === 0">
              <td colspan="9" class="px-4 py-10 text-center text-gray-500 text-sm">ไม่พบข้อมูลลูกค้า</td>
            </tr>
          </tbody>
        </table>
      </div>
...
import { Plus, ChevronDown, ChevronUp, ChevronLeft, ChevronRight, Pencil, ToggleLeft, ToggleRight } from 'lucide-vue-next'
...
export default {
  name: 'Patients',
  components: { 
    Plus, ChevronDown, ChevronUp, ChevronLeft, ChevronRight, Pencil, ToggleLeft, ToggleRight, 
    Listbox, ListboxButton, ListboxOptions, ListboxOption, PatientModal 
  },
  data() {
    return {
      loading: false,
  ...
  methods: {
    ...
    async handleSave(data) {
      ...
      const confirm = await Swal.fire({
        ...
        text: isEdit ? `ต้องการบันทึกการเปลี่ยนแปลงของ "${data.first_name} ${data.last_name}" หรือไม่` : `ต้องการสร้างลูกค้า "${data.first_name} ${data.last_name}" หรือไม่`,
        icon: 'question',
      ...
      try {
        if (isEdit) {
          ...
          Swal.fire({ icon: 'success', title: 'แก้ไขลูกค้าสำเร็จ', timer: 1600, showConfirmButton: false, toast: true, position: 'top-end' })
          await this.reload()
        } else {
          await patientService.createPatient(data)
          this.modalOpen = false
          this.modalLoading = false
          Swal.fire({ icon: 'success', title: 'สร้างลูกค้าสำเร็จ', timer: 1600, showConfirmButton: false, toast: true, position: 'top-end' })
          await this.reload()
        }
      } catch (error) {
        this.modalLoading = false
        Swal.fire({ icon: 'error', title: 'เกิดข้อผิดพลาด', text: error?.response?.data?.message || error.message || 'ไม่สามารถบันทึกข้อมูลได้' })
      }
    },
    async toggleActive(patient) {
      ...
      try {
        ...
        Swal.fire({ icon: 'error', title: 'อัปเดตสถานะไม่สำเร็จ', text: error?.response?.data?.message || error.message || 'กรุณาลองใหม่อีกครั้ง' })
      }
    }
  },
  mounted() {
