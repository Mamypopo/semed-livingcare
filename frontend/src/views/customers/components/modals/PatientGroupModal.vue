            ...
            <DialogPanel class="w-full max-w-2xl transform rounded-2xl bg-white text-left align-middle shadow-xl transition-all">
              ...
              <div class="px-6 py-5 space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <!-- Name -->
                  <div>
                    <label class="block text-sm font-medium text-gray-700">ชื่อกลุ่มลูกค้า <span class="text-red-500">*</span></label>
                    <input v-model.trim="form.name" type="text" class="mt-1 w-full px-3 py-2 border border-gray-200 rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 hover:border-emerald-300 focus:outline-none" placeholder="เช่น Walk-in, Corporate, VIP" />
                  ...
                  <div>
                    ...
                    <Listbox v-model="form.discount_type" as="div" class="relative">
                      <div>
                        <ListboxButton class="mt-1 w-full px-3 py-2 text-left bg-white border border-gray-200 rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 hover:border-emerald-300 focus:outline-none">
                          <span class="block truncate">{{ getDiscountTypeLabel(form.discount_type) }}</span>
                      ...
                      <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                        <ListboxOptions class="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none">
                          <ListboxOption v-for="discountType in discountTypeOptions" :key="discountType.value" :value="discountType.value" v-slot="{ active, selected }">
                    ...
                    <p v-if="errors.discount_type" class="text-xs text-red-600 mt-1">{{ errors.discount_type }}</p>
                  </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div v-if="form.discount_type">
                    ...
                    <div class="relative mt-1">
                      <input v-model.number="form.discount_amount" type="number" min="0" step="0.01" class="w-full px-3 py-2 border border-gray-200 rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 hover:border-emerald-300 focus:outline-none" :placeholder="form.discount_type === 'percent' ? '10' : '100'" />
                      <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                        <span class="text-gray-500 text-sm">{{ form.discount_type === 'percent' ? '%' : 'บาท' }}</span>
                      </div>
                    </div>
                    <p v-if="errors.discount_amount" class="text-xs text-red-600 mt-1">{{ errors.discount_amount }}</p>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <textarea v-model.trim="form.note" rows="3" class="mt-1 w-full px-3 py-2 border border-gray-200 rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 hover:border-emerald-300 focus:outline-none" placeholder="หมายเหตุเพิ่มเติม..."></textarea>
