  ...
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
      ...
      <div class="flex items-center gap-2">
        <input v-model.trim="query" @input="onFilterInput" type="text" placeholder="ค้นหา รหัส/ชื่อ/หมายเหตุ..." class="px-3 py-2 text-sm border border-gray-200 rounded-md focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 w-56 hover:border-emerald-300 focus:outline-none" />
        ...
        <button class="px-4 py-2 text-sm bg-emerald-600 text-white rounded-md hover:bg-emerald-700 focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2" @click="openCreate">เพิ่มประเภทประกัน</button>
      </div>
    </div>
      ...
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              <th @click="toggleSort('code')" class="px-4 py-2 text-left text-xs font-semibold cursor-pointer select-none text-gray-600">
                <span class="inline-flex items-center">รหัส
                  <ChevronUp v-if="sort==='code' && order==='asc'" class="w-3.5 h-3.5 ml-1 text-emerald-600" />
              ...
              <th @click="toggleSort('name')" class="px-4 py-2 text-left text-xs font-semibold cursor-pointer select-none text-gray-600">
                <span class="inline-flex items-center">ชื่อ
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            <!-- Rows -->
            <template v-if="!loading">
              <tr v-for="insuranceType in insuranceTypes" :key="insuranceType.id" class="hover:bg-gray-50">
              ...
              <td class="px-4 py-2 text-sm text-gray-900 font-medium">
                <div>
                  ...
                  <div v-if="insuranceType.note" class="text-xs text-gray-500">{{ insuranceType.note }}</div>
                </div>
              </td>
              <td class="px-4 py-2 text-sm">
                <span :class="insuranceType.isActive ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-600'" class="px-2 py-1 rounded-full text-xs font-medium">
                  {{ insuranceType.isActive ? 'ใช้งาน' : 'ปิดใช้งาน' }}
                </span>
              </td>
              <td class="px-4 py-2 text-sm text-gray-700 whitespace-nowrap">{{ formatDate(insuranceType.createdAt) }}</td>
              <td class="px-4 py-2 text-sm text-right whitespace-nowrap">
                ...
                <button
                  ...
                  @click="toggleActive(insuranceType)"
                  v-tooltip:top="insuranceType.isActive ? 'ปิดใช้งาน' : 'เปิดใช้งาน'"
                >
                  <ToggleRight v-if="insuranceType.isActive" class="w-3.5 h-3.5" />
                  <ToggleLeft v-else class="w-3.5 h-3.5" />
                  {{ insuranceType.isActive ? 'ปิดใช้งาน' : 'เปิดใช้งาน' }}
                </button>
              </td>
              </tr>
            </template>
...
import InsuranceTypeModal from './components/modals/InsuranceTypeModal.vue'
...
export default {
  name: 'InsuranceTypes',
  components: { 
    ChevronDown, ChevronUp, ChevronLeft, ChevronRight, Pencil, ToggleLeft, ToggleRight, 
    Listbox, ListboxButton, ListboxOptions, ListboxOption, InsuranceTypeModal 
  },
  data() {
    return {
      loading: false,
  ...
  methods: {
    ...
    async handleSave(data) {
      ...
      const confirm = await Swal.fire({
        title: isEdit ? 'ยืนยันการแก้ไขประเภทประกัน?' : 'ยืนยันการสร้างประเภทประกันใหม่?',
        text: isEdit ? `ต้องการบันทึกการเปลี่ยนแปลงของ "${data.name}" หรือไม่` : `ต้องการสร้างประเภทประกัน "${data.name}" หรือไม่`,
        icon: 'question',
      ...
      try {
        if (isEdit) {
          ...
          Swal.fire({ icon: 'success', title: 'แก้ไขประเภทประกันสำเร็จ', timer: 1600, showConfirmButton: false, toast: true, position: 'top-end' })
          ...
          this.modalOpen = false
          this.modalLoading = false
          Swal.fire({ icon: 'success', title: 'สร้างประเภทประกันสำเร็จ', timer: 1600, showConfirmButton: false, toast: true, position: 'top-end' })
          // Reload data to get latest information
          await this.reload()
        }
      } catch (e) {
    ...
    async toggleActive(insuranceType) {
      const desired = !insuranceType.isActive
      const res = await Swal.fire({
        title: desired ? 'เปิดใช้งานประเภทประกัน?' : 'ปิดใช้งานประเภทประกัน?',
        text: `${desired ? 'เปิดให้ใช้งาน' : 'ปิดการใช้งาน'} "${insuranceType.name}" หรือไม่`,
        icon: 'question',
        showCancelButton: true,
      ...
      try {
        await insuranceTypeService.updateActive(insuranceType.id, desired)
