<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
      <div>
        <p class="text-md text-gray-500">จัดการข้อมูลกลุ่มลูกค้าทั้งหมดในระบบ</p>
      </div>
      <div class="flex items-center gap-2">
        ...
        <button class="px-4 py-2 text-sm bg-emerald-600 text-white rounded-md hover:bg-emerald-700 focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2" @click="openCreate">เพิ่มกลุ่มลูกค้า</button>
      </div>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-4 py-2 text-left text-xs font-semibold text-gray-600">ส่วนลด</th>
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            <!-- Rows -->
            <template v-if="!loading">
              <tr v-for="group in patientGroups" :key="group.id" class="hover:bg-gray-50">
              <td class="px-4 py-2 text-sm text-gray-900 font-medium">
                <div class="flex items-center">
                  ...
                  <div>
                    <div>{{ group.name }}</div>
                    <div v-if="group.note" class="text-xs text-gray-500">{{ group.note }}</div>
                  </div>
                </div>
              </td>
              <td class="px-4 py-2 text-sm text-gray-700">
                <div v-if="group.color" class="w-6 h-6 rounded-full border border-gray-300" :style="{ backgroundColor: group.color }"></div>
                <span v-else class="text-gray-400">-</span>
              </td>
              <td class="px-4 py-2 text-sm text-gray-700">
                <div v-if="group.discount_type && group.discount_amount">
                  <span v-if="group.discount_type === 'percent'">{{ group.discount_amount }}%</span>
                  <span v-else>{{ group.discount_amount }} บาท</span>
                </div>
                <span v-else class="text-gray-400">-</span>
              </td>
              <td class="px-4 py-2 text-sm">
                <span :class="group.isActive ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-600'" class="px-2 py-1 rounded-full text-xs font-medium">
                  {{ group.isActive ? 'ใช้งาน' : 'ปิดใช้งาน' }}
                </span>
              </td>
              <td class="px-4 py-2 text-sm text-gray-700 whitespace-nowrap">{{ formatDate(group.createdAt) }}</td>
...
export default {
  ...
  components: { 
    ...
    Listbox, ListboxButton, ListboxOptions, ListboxOption, PatientGroupModal 
  },
  data() {
    return {
      loading: false,
  ...
  methods: {
    ...
    async handleSave(data) {
      ...
      const confirm = await Swal.fire({
        ...
        text: isEdit ? `ต้องการบันทึกการเปลี่ยนแปลงของ "${data.name}" หรือไม่` : `ต้องการสร้างกลุ่มลูกค้า "${data.name}" หรือไม่`,
        icon: 'question',
      ...
      try {
        if (isEdit) {
          ...
          Swal.fire({ icon: 'success', title: 'แก้ไขกลุ่มลูกค้าสำเร็จ', timer: 1600, showConfirmButton: false, toast: true, position: 'top-end' })
          ...
          this.modalOpen = false
          this.modalLoading = false
          Swal.fire({ icon: 'success', title: 'สร้างกลุ่มลูกค้าสำเร็จ', timer: 1600, showConfirmButton: false, toast: true, position: 'top-end' })
          // Reload data to get latest information
          await this.reload()
        }
      } catch (e) {
