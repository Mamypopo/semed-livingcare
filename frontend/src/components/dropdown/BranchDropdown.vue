      ...
      <div class="relative">
        <ListboxButton class="relative w-full cursor-default rounded-lg bg-white py-2 pl-3 pr-10 text-left shadow-sm border border-gray-200 text-gray-700 focus:border-emerald-400 focus:ring-1 focus:ring-emerald-300/80 focus:outline-none transition-colors duration-200 hover:border-emerald-400">
          <div class="flex items-center gap-2">
            <span class="block truncate">{{ selectedBranch ? selectedBranch.name : placeholder }}</span>
            <span v-if="selectedBranch?.code" class="px-2 py-1 text-xs bg-blue-100 text-blue-700 rounded-md font-medium whitespace-nowrap">
              {{ selectedBranch.code }}
            </span>
          </div>
        ...
        <ListboxOptions class="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
            ...
            <li :class="[active ? 'bg-emerald-100 text-emerald-900' : 'text-gray-900', 'relative cursor-default select-none py-2 pr-4', selected ? 'pl-10' : 'pl-3']">
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                  ...
                  <span v-if="branch.code" class="px-2 py-1 text-xs bg-blue-100 text-blue-700 rounded-md font-medium whitespace-nowrap">
                    {{ branch.code }}
                  </span>
                </div>
          ...
          <!-- No Results -->
          <div v-if="filteredBranches.length === 0" class="px-3 py-2 text-sm text-gray-500 text-center">
            ไม่พบสาขา
          </div>
        </ListboxOptions>
      </div>
